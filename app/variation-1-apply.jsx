...
function V1ApplicationFlow({
  ...
  // Captive retention: bailing out before the offer is accepted throws up
  // an "Are you sure?" gate rather than closing straight away.
  const [showLeaveConfirm, setShowLeaveConfirm] = React.useState(false);

  React.useEffect(() => {
    if (open) { setStep(startStep); setClosing(false); setShowLeaveConfirm(false); }
  }, [open, startStep]);
  ...
  const handleClose = () => {
    // Once the offer's been accepted (Done step) there's nothing to retain —
    // let them out. Otherwise intercept with the "Are you sure?" gate.
    if (step >= 4) { doClose(); return; }
  ...
  // ─── Captive retention gate ───────────────────────────────────────
  // Shown over the modal when the user tries to leave mid-application.
  const leaveConfirm = showLeaveConfirm ? (
    ...
    >
      <div style={{
        ...
        background: V1.bg, borderRadius: 18,
        border: `1px solid ${V1.line}`,
        boxShadow: '0 40px 100px -20px rgba(15,14,23,0.55)',
        padding: '30px 30px 26px', textAlign: 'center',
        ...
        <div style={{
          ...
          background: `linear-gradient(135deg, ${accent}, #818CF8)`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: V1.white, boxShadow: `0 10px 26px -10px ${accent}aa`,
        }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M12 2L3 7v6c0 5 3.8 8.3 9 9 5.2-.7 9-4 9-9V7l-9-5z"/>
          </svg>
        </div>
        <h3 style={{
          margin: 0, fontFamily: V1.fontDisplay, fontSize: 22, fontWeight: 700,
          letterSpacing: '-0.02em', color: V1.ink,
        }}>Are you sure?</h3>
        <p style={{
          margin: '10px 0 0', fontFamily: V1.fontBody, fontSize: 14.5,
          lineHeight: 1.55, color: V1.muted,
        }}>
          You're moments from your funding offer. Leave now and you'll have to
          start over — your range and details won't be saved.
        </p>
  ...
  return ReactDOM.createPortal(
    <React.Fragment>{modal}{leaveConfirm}</React.Fragment>,
    document.body,
  );
}
