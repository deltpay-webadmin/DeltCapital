...
export function Chatbot({ onApplyClick, onCalculatorClick, onBookingClick, onSupportClick }: ChatbotProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {isExpanded ? (
        <div className="bg-white rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.12)] w-[380px] overflow-hidden border border-[#0F0E17]/8">
          {/* Header — refined institutional */}
          <div className="bg-gradient-to-r from-[#0F0E17] to-[#062a5a] text-white px-4 py-4 flex items-center justify-between">
          ...
          <div className="p-5 max-h-[520px] overflow-y-auto bg-[#fafbfc]">
            {/* Time stamp — minimal */}
            <div className="text-center text-[11px] text-[#0F0E17]/40 font-medium tracking-wide mb-5 uppercase">
              {formatTime()}
            </div>

            {/* Bot Message — refined card */}
            <div className="flex items-start gap-3 mb-6">
              <div className="w-8 h-8 bg-gradient-to-br from-[#4F46E5] to-[#3730A3] rounded-md flex items-center justify-center flex-shrink-0 shadow-sm">
                <MessageCircle className="w-4 h-4 text-white" strokeWidth={2} />
              </div>
              <div className="bg-white border border-[#0F0E17]/6 rounded-lg rounded-tl-none px-4 py-3.5 flex-1 shadow-sm">
                <p className="text-[13px] leading-relaxed text-[#0F0E17]/85" style={{ fontWeight: 450 }}>
                  {language === 'es' 
            ...
            <div className="space-y-2 ml-11">
              <button
                ...
                className="group w-full bg-white hover:bg-[#4F46E5] border border-[#0F0E17]/8 hover:border-[#4F46E5] text-[#0F0E17] hover:text-white rounded-md px-4 py-3 text-[13px] transition-all duration-200 text-left shadow-sm hover:shadow-md"
              ...
              <div key={message.id} className={`flex items-start gap-3 mb-4 mt-2 ${message.type === 'user' ? 'justify-end' : ''}`}>
                ...
                <div
                  className={`px-4 py-3.5 shadow-sm max-w-[80%] ${
                    message.type === 'user' 
                      ? 'bg-[#4F46E5] border border-[#4F46E5] rounded-lg rounded-tr-none' 
                      : 'bg-white border border-[#0F0E17]/6 rounded-lg rounded-tl-none'
                  }`}
                >
                  <p className={`text-[13px] leading-relaxed ${
                    message.type === 'user' ? 'text-white' : 'text-[#0F0E17]/85'
          ...
          <div className="px-5 py-3 bg-white border-t border-[#0F0E17]/6 flex items-center">
            <input
              type="text"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder={language === 'es' ? 'Escribe un mensaje...' : 'Type a message...'}
              className="w-full bg-[#fafbfc] border border-[#0F0E17]/6 rounded-lg px-4 py-3 text-[13px] leading-relaxed text-[#0F0E17]/85"
              onKeyPress={handleKeyPress}
            />
            <button
              onClick={sendMessage}
              className="ml-3 bg-[#4F46E5] hover:bg-[#3730A3] text-white rounded-md px-4 py-3 text-[13px] transition-all duration-200 shadow-sm hover:shadow-md"
            >
          ...
          {/* Footer — subtle */}
          <div className="px-5 py-3 bg-white border-t border-[#0F0E17]/6">
            <p className="text-[11px] text-[#0F0E17]/50 text-center font-medium tracking-wide">
              {language === 'es' 
        ...
        <button
          ...
          className="bg-gradient-to-br from-[#0F0E17] to-[#062a5a] hover:from-[#062a5a] hover:to-[#0F0E17] text-white rounded-lg w-14 h-14 flex items-center justify-center shadow-[0_4px_24px_rgba(4,30,66,0.25)] transition-all duration-300 hover:scale-105 hover:shadow-[0_8px_32px_rgba(4,30,66,0.35)] relative group"
          aria-label="Open chat"
        >
          <MessageCircle className="w-6 h-6 transition-transform group-hover:scale-110" strokeWidth={2} />
          <span className="absolute -top-1 -right-1 bg-[#4F46E5] text-white text-[10px] font-bold rounded-full w-[18px] h-[18px] flex items-center justify-center shadow-sm border-2 border-white">
            1
          </span>
        </button>
      )}
