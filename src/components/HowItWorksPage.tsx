...
export function HowItWorksPage({ onClose, onApplyClick, onCalculatorClick, onAboutClick, onHowItWorksClick, onReviewsClick, onBlogClick, onFAQClick, onSupportClick, onWinsClick, onPrivacyClick, onTermsClick, onDisclosuresClick, onResourcesClick }: HowItWorksPageProps) {
  ...
  return (
    <div className="fixed inset-0 bg-[#F7F5F0] z-50 flex flex-col">
      {/* Spacer for navbar */}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <section className="pt-20 pb-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto text-center">
            <motion.h1
              ...
              className="text-4xl sm:text-5xl md:text-6xl text-[#0F0E17] mb-5 tracking-tight"
              style={{ fontWeight: 700 }}
            >
              How it <em className="text-[#4F46E5] not-italic" style={{ fontStyle: 'italic' }}>works</em>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 14 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.15 }}
              className="text-[#0F0E17]/60 max-w-xl mx-auto"
              ...
              {steps.map((step, i) => (
                <div key={step.number} className="contents">
                  ...
                  >
                    ...
                    <motion.div
                      className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors duration-300 ${
                        expandedStep === step.number
                          ...
                          : 'bg-[#4F46E5]/10 text-[#4F46E5] group-hover:bg-[#4F46E5]/25'
                      }`}
                      whileHover={{
                        scale: 1.12,
                    ...
                    <h3
                      className="text-[#0F0E17] text-center mb-2 group-hover:text-[#4F46E5] transition-colors duration-200"
                      style={{ fontWeight: 600, fontSize: '1.05rem' }}
                    >
                      {step.title}
                    </h3>
                    <p className="text-[#0F0E17]/50 text-center" style={{ fontSize: '0.85rem', lineHeight: 1.55 }}>
            ...
            <div className="md:hidden space-y-4">
              {steps.map((step, i) => (
                ...
                >
                  ...
                  <div>
                    <h3 className="text-[#0F0E17] mb-1" style={{ fontWeight: 600 }}>{step.title}</h3>
                    <p className="text-[#0F0E17]/50" style={{ fontSize: '0.85rem', lineHeight: 1.5 }}>
        ...
        <section className="pb-24 px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-2xl sm:text-3xl text-[#0F0E17] mb-3" style={{ fontWeight: 700 }}>
              Ready to get started?
            </h2>
            <p className="text-[#0F0E17]/50 mb-8" style={{ fontSize: '0.95rem' }}>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-[#4F46E5] hover:bg-[#3730A3] text-white px-8 py-3.5 rounded-lg transition-all duration-200 hover:shadow-lg hover:shadow-[#4F46E5]/25"
              style={{ fontWeight: 600, fontSize: '1rem' }}
            >
              Check my amount
              <ArrowRight className="w-5 h-5" />
            </button>
            <p className="mt-4 text-[#0F0E17]/40" style={{ fontSize: '0.8rem' }}>
              ...
              >
                <div className="p-7 sm:p-9">
                  ...
                  <button
                    ...
                    className="absolute top-5 right-5 w-9 h-9 rounded-full bg-[#F7F5F0] hover:bg-[#E4E7F0] flex items-center justify-center transition-colors z-10"
                  >
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <motion.div
                      ...
                      className="w-14 h-14 rounded-full bg-[#4F46E5] text-white flex items-center justify-center flex-shrink-0"
                    >
                      <span className="text-xl" style={{ fontWeight: 700 }}>{step.number}</span>
                    </motion.div>
                    <div>
                      <p className="text-[#4F46E5] text-xs tracking-wider mb-0.5" style={{ fontWeight: 600 }}>
                        STEP {step.number}
                      </p>
                      <h2 className="text-[#0F0E17] text-xl sm:text-2xl" style={{ fontWeight: 700 }}>
                        {step.title}
                      </h2>
                    </div>
                  </div>
                  ...
                  {/* Tagline */}
                  <p className="text-[#4F46E5] mb-4" style={{ fontWeight: 600, fontSize: '1rem' }}>
                    {step.tagline}
                  </p>

                  {/* Description */}
                  <p className="text-[#0F0E17]/60 mb-8" style={{ fontSize: '0.95rem', lineHeight: 1.7 }}>
                    {step.description}
                  </p>
                  ...
                  <div className="space-y-4 mb-8">
                    {step.bullets.map((bullet, idx) => (
                      ...
                      >
                        <div className="w-10 h-10 rounded-xl bg-[#4F46E5]/[0.06] flex items-center justify-center flex-shrink-0 text-[#4F46E5]">
                          {bullet.icon}
                        </div>
                        <p className="text-[#0F0E17]/80 pt-2" style={{ fontSize: '0.93rem' }}>
                          {bullet.text}
                        </p>
                      </motion.div>
                    ))}
                  ...
                  <motion.div
                    ...
                    className="bg-[#FFFFFF] rounded-xl px-6 py-4 flex items-center gap-4"
                  >
                    <span className="text-[#4F46E5] text-3xl" style={{ fontWeight: 700 }}>
                      {step.stat}
                    </span>
                    <span className="text-[#0F0E17]/50" style={{ fontSize: '0.9rem' }}>
                      {step.statLabel}
                    </span>
                  </motion.div>
                </div>
