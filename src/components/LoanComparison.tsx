...
export function LoanComparison() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-200">
      <div className="mb-8">
        <h3 className="text-2xl font-bold text-[#172b4d] mb-2">MCA vs. Bank Loan Comparison</h3>
        <p className="text-gray-600">Understanding the key differences to make the right choice</p>
      </div>

      {/* Quick Summary */}
      <div className="grid md:grid-cols-2 gap-6 mb-12">
        <div className="bg-gradient-to-br from-[#0c66e4] to-[#0055cc] rounded-xl p-6 text-white">
        ...
        <div className="bg-gradient-to-br from-[#172b4d] to-[#0a2d5a] rounded-xl p-6 text-white">
      ...
      {comparisonFeatures.map((section, sectionIndex) => (
        <div key={sectionIndex} className="mb-8">
          <h4 className="text-xl font-bold text-[#172b4d] mb-4">{section.category}</h4>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  <th className="text-left py-3 px-4 font-semibold text-gray-700">Feature</th>
                  <th className="text-center py-3 px-4 font-semibold text-[#0c66e4]">Merchant Cash Advance</th>
                  <th className="text-center py-3 px-4 font-semibold text-[#172b4d]">Bank Loan</th>
                </tr>
              </thead>
              <tbody>
                {section.features.map((feature, index) => (
                  <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className={`py-4 px-4 text-center ${feature.winner === 'mca' ? 'bg-blue-50 font-semibold text-[#0c66e4]' : 'text-gray-600'}`}>
                      {feature.mca}
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-12">
        <div className="bg-blue-50 border-2 border-[#0c66e4] rounded-xl p-6">
          ...
          <ul className="space-y-2">
            {whenToChoose.mca.map((item, index) => (
              <li key={index} className="flex items-start gap-2 text-gray-700">
                <CheckCircle2 className="w-5 h-5 text-[#0c66e4] flex-shrink-0 mt-0.5" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
      ...
      {/* Bottom CTA */}
      <div className="mt-8 bg-gradient-to-r from-[#0c66e4] to-[#0055cc] rounded-xl p-8 text-white text-center">
        ...
        <div className="flex gap-4 justify-center flex-wrap">
          <button className="bg-white text-[#0c66e4] px-8 py-3 rounded-xl font-semibold hover:bg-gray-100 transition-colors">
            Apply for MCA
          </button>
