...
const totalPaidData = [
  { name: 'Principal', value: PRINCIPAL_PAID, color: '#0c66e4' },
  { name: 'Factor Fees', value: FEES_PAID, color: '#F97316' },
];

const payoffData = [
  { name: 'Principal', value: PRINCIPAL_REMAINING, color: '#0c66e4' },
...
export function LoanDashboard({ userEmail, onLogout }: LoanDashboardProps) {
  ...
  return (
    <div className="fixed inset-0 z-[55] bg-[#fafbfc] flex flex-col">
      {/* Spacer for main Navbar (z-70) */}
      <div className="flex-shrink-0 h-[73px]" />

      {/* Dashboard header — sticky below main Navbar */}
      <header className="flex-shrink-0 bg-[#f1f2f4] border-b border-[#0c66e40F] shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-14">
            <div className="flex items-center gap-4">
              <img src={logoImg} alt="Delt" className="h-8 w-auto object-contain" />
              <div className="hidden sm:flex items-center gap-2">
                <span className="font-mono font-semibold text-[#172b4d] dark:text-white">{loanData.loanId}</span>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 pl-3 border-l border-gray-200 dark:border-gray-600">
                <div className="w-8 h-8 rounded-full bg-[#0c66e4] flex items-center justify-center text-white text-sm font-semibold">
              ...
              <button
                onClick={onLogout}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold text-[#0c66e4] bg-[#0c66e4]/10 hover:bg-[#0c66e4]/20 transition-colors"
              >
      ...
      <div className="flex-shrink-0 bg-[#f1f2f4] border-b border-[#0c66e40F]">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard
          ...
          accent="blue"
        />
        <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-5 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-green-500" />
            <span className="text-sm text-gray-500 dark:text-gray-400">AutoPay Enabled</span>
          </div>
          <p className="text-sm text-[#172b4d] dark:text-white">
        ...
        <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-5 flex items-center justify-center">
          <button className="w-full py-3 bg-[#0c66e4] hover:bg-[#0055cc] text-white font-semibold rounded-xl transition-colors shadow-lg shadow-[#0c66e4]/20 flex items-center justify-center gap-2">
      ...
      {/* Loan Balance Bar */}
      <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-[#172b4d] dark:text-white">Repayment Progress</h3>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 mb-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-[#172b4d] dark:text-white">${fmt(loanData.totalRepayment)}</p>
          ...
          <span className="text-2xl text-gray-300">=</span>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#0c66e4]">${fmt(loanData.totalRemaining)}</p>
            <p className="text-xs text-gray-400">{loanData.paymentsRemaining} Payments Left</p>
          </div>
        </div>
        <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
          <div
            className="h-full rounded-full bg-gradient-to-r from-[#0c66e4] to-[#8B5CF6] transition-all duration-500"
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          <div>
            <p className="text-sm font-semibold text-[#172b4d] dark:text-white">${fmt(ORIGINAL_ADVANCE)}</p>
            <p className="text-xs text-gray-400">Original Advance</p>
          </div>
          <div>
            <p className="text-sm font-semibold text-[#172b4d] dark:text-white">${fmt(COST_OF_CAPITAL)}</p>
          ...
          <div>
            <p className="text-sm font-semibold text-[#0c66e4]">${fmt(PRINCIPAL_REMAINING)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-[#172b4d] dark:text-white">Transactions</h3>
            ...
            {displayedTransactions.map((tx, i) => (
              <div key={`${tx.date}-${i}`} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-[#172b4d] dark:text-white">{tx.type}</p>
          ...
          <button
            ...
            className="mt-4 text-sm text-[#0c66e4] hover:text-[#0055cc] font-semibold transition-colors"
          >
        ...
        {/* Total Paid Donut */}
        <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
          <h3 className="font-semibold text-[#172b4d] dark:text-white mb-2">Total Paid</h3>
          ...
          <div className="relative h-[220px] flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-xl font-bold text-[#172b4d] dark:text-white">
                ${fmt(TOTAL_PAID)}
              </span>
        ...
        <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
          <h3 className="font-semibold text-[#172b4d] dark:text-white mb-2">Payoff Breakdown</h3>
...
function KPICard({ label, value, sub, accent }: { label: string; value: string; sub: string; accent: 'green' | 'blue' }) {
  return (
    <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-5">
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{label}</p>
      <p className={`text-2xl font-bold ${accent === 'green' ? 'text-green-600' : 'text-[#172b4d] dark:text-white'}`}>{value}</p>
...
function PaymentsTab() {
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
        <h3 className="font-semibold text-[#172b4d] dark:text-white mb-2">PAYMENT HISTORY</h3>
            ...
            <tbody>
              {Object.entries(paymentHistory).sort(([a],[b]) => Number(b) - Number(a)).map(([year, data]) => (
                <tr key={year} className="border-t border-gray-100 dark:border-gray-700">
                  <td className="py-2.5 pr-4 font-semibold text-[#172b4d] dark:text-white">{year}</td>
                  {data.map((cell, i) => (
      ...
      {/* Payment Details */}
      <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
        <h3 className="font-semibold text-[#172b4d] dark:text-white mb-4">PAYMENT DETAILS</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0">
          {paymentDetails.map((item, i) => (
            <div key={i} className="flex justify-between py-3 border-b border-gray-100 dark:border-gray-700">
              ...
              <span className="text-sm font-semibold text-[#172b4d] dark:text-white">{item.value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
        <h3 className="font-semibold text-[#172b4d] dark:text-white mb-4">LOAN SUMMARY</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0">
          {[
...
function DocumentsTab() {
  ...
  return (
    <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
      <h3 className="font-semibold text-[#172b4d] dark:text-white mb-5">Documents</h3>
      <div className="space-y-0">
        {docs.map((doc, i) => (
          <div key={i} className="flex items-center justify-between py-4 border-b border-gray-100 dark:border-gray-700 last:border-0">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#0c66e4]/10 flex items-center justify-center">
                <FileText className="w-5 h-5 text-[#0c66e4]" />
              </div>
              <div>
                <p className="text-sm font-semibold text-[#172b4d] dark:text-white">{doc.name}</p>
                <p className="text-xs text-gray-400">{doc.type} · {doc.date}</p>
              </div>
            </div>
            <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-[#0c66e4] hover:bg-[#0c66e4]/10 rounded-lg transition-colors font-semibold">
...
function SettingsTab({ userEmail }: { userEmail: string }) {
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
        <h3 className="font-semibold text-[#172b4d] dark:text-white mb-5">Account Settings</h3>
        <div className="space-y-4">
          <div className="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-700">
            <div>
              <p className="text-sm font-semibold text-[#172b4d] dark:text-white">Email Address</p>
              <p className="text-sm text-gray-400">{userEmail}</p>
            </div>
            <button className="text-sm text-[#0c66e4] font-semibold">Edit</button>
          </div>
          <div className="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-700">
            <div>
              <p className="text-sm font-semibold text-[#172b4d] dark:text-white">Password</p>
              <p className="text-sm text-gray-400">Last changed 30 days ago</p>
            </div>
            <button className="text-sm text-[#0c66e4] font-semibold">Change</button>
          </div>
          <div className="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-700">
            <div>
              <p className="text-sm font-semibold text-[#172b4d] dark:text-white">AutoPay</p>
              <p className="text-sm text-gray-400">Enabled — Bank account ending in 4821</p>
            </div>
            <button className="text-sm text-[#0c66e4] font-semibold">Manage</button>
          </div>
          <div className="flex justify-between items-center py-3">
            <div>
              <p className="text-sm font-semibold text-[#172b4d] dark:text-white">Notifications</p>
              <p className="text-sm text-gray-400">Email & SMS reminders enabled</p>
            </div>
            <button className="text-sm text-[#0c66e4] font-semibold">Configure</button>
          </div>
        </div>
      </div>

      <div className="bg-[#f1f2f4] rounded-xl border border-[#0c66e40F] p-6">
        <h3 className="font-semibold text-[#172b4d] dark:text-white mb-3">Need Help?</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Contact our support team for any questions about your loan.
        </p>
        <div className="flex items-center gap-3 text-sm">
          <HelpCircle className="w-5 h-5 text-[#0c66e4]" />
          <span className="text-[#172b4d] dark:text-white font-semibold">(864) 729-3358</span>
        </div>
      </div>
    </div>
  );
