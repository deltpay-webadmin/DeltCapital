...
export function MonthlyRevenueSlider({ value, onChange }: MonthlyRevenueSliderProps) {
  ...
  useEffect(() => { onChange(revenue); }, [revenue, onChange]);
  ...
  const minVal = REVENUE_STOPS[0];
  const maxVal = REVENUE_STOPS[REVENUE_STOPS.length - 1];
