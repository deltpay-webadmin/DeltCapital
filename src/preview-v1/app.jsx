...
import { DELT, DeltContent, Btn, Tick, Ex, Pill, calcEstimate, fmt, Divider, Arr } from './shared';
...
function FooterBlock({ accent = DELT.colors.indigo, brand }) {
  return (
    <footer style={{ background: DELT.colors.ink, color: '#E9E7DF', padding: '64px 32px 40px' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 48, marginBottom: 56 }}>
          ...
          {[
            { h: 'Product', links: ['Funding', 'Calculator', 'How it works', 'Pricing'] },
            { h: 'Company', links: ['About', 'Reviews', 'Stories', 'Press'] },
            { h: 'Resources', links: ['FAQ', 'Blog', 'Support', 'Contact'] },
          ].map(col => (
            <div key={col.h}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {col.links.map(l => (
                  <a key={l} style={{ fontFamily: DELT.font.body, fontSize: 14, color: '#E9E7DF', textDecoration: 'none', cursor: 'pointer' }}>{l}</a>
                ))}
              </div>
            </div>
          ))}
        </div>
        <div style={{ paddingTop: 28, borderTop: '1px solid #2B2A35', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, fontFamily: DELT.font.body, fontSize: 12, color: '#6A6876' }}>
          <div>© 2026 Delt Capital, Inc. NMLS #—. California Finance Lender License #—.</div>
          <div style={{ display: 'flex', gap: 20 }}>
            <span>Terms</span><span>Privacy</span><span>Disclosures</span><span>Licenses</span>
          </div>
        </div>
      </div>
    </footer>
